...
def load_comparator(rule: str) -> Callable:
  ...
  if comparator is None or not callable(comparator):
    raise ValueError(f"Cannot find callable {module_name}.{func_name}")
...
def parse_interval(objects: list[dict], interval: str) -> tuple[int, int]:
  ...
  if len(parts) != 2:
    raise ValueError(f"Invalid interval syntax {interval!r}, expected BEFORE:AFTER")
  before_id, after_id = parts

  if before_id == "*":
    start = 0
  else:
    idx = next((i for i, o in enumerate(objects) if o["id"] == before_id), None)
    if idx is None:
      raise ValueError(f"Interval boundary {before_id!r} not found in objects")
    start = idx + 1

  if after_id == "*":
    end = len(objects)
  else:
    idx = next((i for i, o in enumerate(objects) if o["id"] == after_id), None)
    if idx is None:
      raise ValueError(f"Interval boundary {after_id!r} not found in objects")
    end = idx

  if start > end:
    raise ValueError(f"Invalid interval {interval!r}: start ({start}) > end ({end})")
  return (start, end)


def resolve_intervals(
  objects: list[dict], intervals: list[str]
) -> list[tuple[int, int]]:
  ...
  for i in range(1, len(ranges)):
    if ranges[i][0] < ranges[i - 1][1]:
      raise ValueError(
        f"Overlapping intervals: indices {ranges[i - 1]} and {ranges[i]}"
      )
  return ranges


def apply_intervals(
  objects: list[dict], ranges: list[tuple[int, int]], sort_key: Callable
) -> list[dict]:
...
def sort_composite_rows(
  ...
  sort_key = make_sort_key(comparator, template_row, context, args=sort_args, rows=rows)
