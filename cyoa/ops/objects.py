...
def list_objects(project: dict, row_id: str) -> list[ObjectEntry]:
  ...
  for idx, obj in enumerate(row_data["objects"]):
    ...
    for score in obj["scores"]:
      ...
      val = int(score["value"])
      scores[pt_name] = ScoreInfo(value=abs(val), sign="Gain" if val < 0 else "Cost")
    result.append(
      ObjectEntry(index=idx, object_id=obj["id"], title=obj["title"], scores=scores)
    )
  return result
...
def insert_objects_in_row(
  ...
  if after_idx is not None:
    ...
    insert_index = find_first_index(
      row_data["objects"], lambda obj: obj["id"] == after_obj
    )
