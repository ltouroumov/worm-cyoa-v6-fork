...
def list_all_images(project: dict) -> Iterator[ImageInfo]:
    ...
    for row in project['rows']:
        if rowImage := extract_image_from_item(row):
            yield replace(rowImage, object_type="row", row_id=row.get('id'),
                          name=row.get('title'))

        for style_prop in IMAGE_PROPS:
            if bgImage := extract_image_from_style(row, prop=style_prop):
                yield replace(bgImage, object_type="row", row_id=row.get('id'),
                              style_prop=style_prop)

        for obj in row['objects']:
            if objImage := extract_image_from_item(obj):
                yield replace(objImage, object_type="obj", row_id=row.get('id'),
                              obj_id=obj.get('id'), name=obj.get('title'))

            for style_prop in IMAGE_PROPS:
                if bgImage := extract_image_from_style(obj, prop=style_prop):
                    yield replace(bgImage, object_type="obj", row_id=row.get('id'),
                                  obj_id=obj.get('id'), style_prop=style_prop)
...
def optimize_image(image: Image.Image, max_size: Optional[Tuple[int, int]] = None) -> bytes:
...
def _update_with_lens(project: dict, row_id: Optional[str], obj_id: Optional[str], lens_op):
...
def _update_row_data(project: dict, row_id: str, lens_op):
    ...
    if (row_idx := find_first_index(project['rows'],
                                     lambda row: row.get('id') == row_id)) is None:
        return
...
def _update_obj_data(project: dict, row_id: str, obj_id: str, lens_op):
    ...
    row_data = project['rows'][row_idx]
    if (obj_idx := find_first_index(row_data['objects'],
                                     lambda obj: obj.get('id') == obj_id)) is None:
        return
...
def update_image(project: dict, image_info: ImageInfo,
