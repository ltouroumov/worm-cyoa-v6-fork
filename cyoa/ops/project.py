...
def check_duplicates(
  project: dict, graph: Optional[Graph] = None
) -> tuple[list[DuplicateIssue], list[EmptyIdIssue]]:
  ...
  for obj_id, titles in object_ids.items():
    if len(titles) > 1:
      ...
      inputs = [(input_id, graph.objects[input_id].title) for input_id in vertex.inputs]
      outputs = [
        (output_id, graph.objects[output_id].title) for output_id in vertex.outputs
      ]
...
def check_project(
  project: dict, ignored_rows: Optional[set[str]] = None
) -> CheckResult:
...
def collect_power_rows(
  project: dict, graph: Graph, power_row_ids: set[str]
) -> list[PowerEntry]:
...
def _get_requirements(reqs: Optional[Condition], graph: Graph, root: bool) -> list[str]:
  ...
  match reqs:
    case AndCondition(terms) if root:
      return [
        req for term in terms for req in _get_requirements(term, graph, root=False)
      ]
...
def _add_scores(totals: dict[str, Score], scores: Sequence[Score]):
  ...
  for score in scores:
    if score.points_id not in max_scores:
      ...
      max_scores[score.points_id] = max(max_scores[score.points_id], score.value)

  for points_id, value in max_scores.items():
    if points_id not in totals:
      totals[points_id] = Score(points_id=points_id, value=0, requirements=None)
