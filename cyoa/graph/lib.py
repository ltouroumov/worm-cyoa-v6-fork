...
def collect_condition_deps(
  ...
  if isinstance(cond, (AndCondition, OrCondition)):
    for item in cond.terms:
      yield from collect_condition_deps(item, path_requirements, path_incompatibles)
...
def collect_object_deps(
  ...
  yield from collect_condition_deps(
    obj.requirements, path_requirements, path_incompatibles
  )
  if path_scores:
    for score in obj.scores:
      yield from collect_condition_deps(
        score.requirements, path_requirements, path_incompatibles
      )
  if path_addons:
    for addon in obj.addons:
      yield from collect_condition_deps(
        addon.requirements, path_requirements, path_incompatibles
      )
...
class Graph:
  ...
  def filter(self, filter=None):
    ...
    rows = {row_id: row for row_id, row in self.rows.items() if row_id in row_ids}
  ...
  def _build_vertices(self):
    ...
    for obj in self.objects.values():
      self._vertices[obj.obj_id] = Vertex(obj.obj_id, set(), set())

    deps = (
      (obj.obj_id, dep)
      for obj in self.objects.values()
      for dep in collect_object_deps(
        obj,
        path_requirements=self.path_requirements,
        path_incompatibles=self.path_incompatibles,
        path_scores=self.path_scores,
...
def build_graph(
  ...
  def build_requirement(data) -> Condition:
    if data["type"] == "id" and data["required"]:
      ...
      return OrCondition(
        [RequiredCondition(item["req"]) for item in data["orRequired"] if item["req"]]
      )
    else:
  ...
  def build_addon(data) -> Addon:
    return Addon(
      title=data["title"], requirements=build_requirements(data["requireds"])
    )
