...
from cyoa.ops.sort import (
    SortContext, load_comparator, sort_row, sort_composite_rows
)
        ...
        if lint:
            if result.is_sorted:
                ...
                for diff in result.lint_diffs:
                    ...
                    table.add_row(str(diff.index), diff.current_title,
                                marker, diff.expected_title)
                console.print(table)
    ...
    def _sort_composite_rows(self, row_ids, max_objects, rule, sort_args,
        ...
        try:
            result = sort_composite_rows(
                self.project, row_ids, max_objects, comparator,
                sort_args=sort_args, intervals=intervals,
