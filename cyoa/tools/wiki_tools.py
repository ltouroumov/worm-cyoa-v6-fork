...
class WikiAPI:
  ...
  def _query_raw(self, **kwargs):
    query_res = self.session.get(
      URL_BASE, params={"action": "query", "format": "json", **kwargs}
    )
  ...
  def query_prefix(self, prefix: str, **kwargs):
    ...
    while can_continue:
      query_res = self._query_raw(
        list="allpages", apprefix=prefix, apcontinue=do_continue, **kwargs
      )
  ...
  def query_content(self, **kwargs):
    query_res = self._query_raw(
      prop="revisions", rvslots="*", rvprop="ids|content", **kwargs
    )
    return query_res["query"]["pages"]

  def parse(self, **kwargs):
    query_res = self.session.get(
      URL_BASE, params={"action": "parse", "format": "json", **kwargs}
    )
...
def remove_indent(text: str) -> str:
  lines = text.split("\n")
  return str.join("\n", (str.lstrip(line) for line in lines))
...
def child_pages_of(pages: list, path: str, search_depth: int = 1):
  ...
  return list(
    filter(
      lambda item: (
        item["title"] != path
        and str.startswith(item["title"], path)
        and (get_depth(item["title"]) - cur_depth) <= search_depth
      ),
      pages,
    )
  )
...
class WikiUpdateTool(ToolBase, ProjectUtilsMixin):
  ...
  def setup(self, args):
    self.api = WikiAPI()
    self.api.login(args.bot_username, args.bot_password)
    ...
    self.rows = {row["id"]: row for row in self.project["rows"]}

    self.objects = {
      obj["id"]: obj for row in self.rows.values() for obj in row["objects"]
    }

    self.scores = {score["id"]: score for score in self.project["pointTypes"]}
  ...
  def run(self, args):
    ...
    with open(css_file_path, mode="r") as css_file:
      self.api.edit(title="MediaWiki:Common.css", text=css_file.read())

      console.print("Updated: CSS")

    structure_items = filter(
      lambda tup: (
        fnmatch(tup[0], args.filter_path) and tup[1]["mode"] in args.filter_type
      ),
      STRUCTURE.items(),
    )
    ...
    with Progress(*columns) as p, ThreadPoolExecutor() as ex:
  ...
  def collect_index(self, path, config, p):
    ...
    wiki_page_names = filter(
      lambda item: (
        item != path
        and str.startswith(item, path)
        and (get_depth(item) - cur_depth) <= search_depth
      ),
      STRUCTURE.keys(),
    )
    ...
    # Flatten batches
    wiki_pages = itertools.chain.from_iterable(batch.values() for batch in wiki_pages)
    # Sort pages by order
    wiki_pages = sorted(
      wiki_pages,
      key=lambda item: (get_index_order(item["title"], STRUCTURE), item["title"]),
    )
  ...
  def collect_pages(self, path, config, args, p):
    ...
    if len(wiki_pages) > 0:
      wiki_pages: dict = self.api.query_content(
        pageids=str.join("|", [f"{wiki_page['pageid']}" for wiki_page in wiki_pages])
      )
      wiki_pages: dict = {
        wiki_page["title"]: wiki_page for wiki_page in wiki_pages.values()
      }
    else:
      wiki_pages: dict = {}
    ...
    for row_id in config["row_ids"]:
      ...
      for obj_data in row_data["objects"]:
        ...
        page_text = remove_indent(
          page_tpl.render(page_name=page_name, row=row_data, obj=obj_data)
        )
        page_text = splice_text(wiki_page, page_text, marker=f"SYNC:{obj_data['id']}")

        if args.preview:
          console.print(page_text)

        if not matches_last_revision(wiki_page, page_text):
          self.api.edit(title=page_name, text=page_text)
      ...
      page_rows.append({"row": row_data, "objects": row_pages})
  ...
  def build_section(self, path, config, args, p):
    ...
    index_text = remove_indent(
      index_tpl.render(
        page_path=path, page_rows=page_rows, pages=wiki_pages, config=config
      )
    )
...
class WikiPowersIndexTool(ToolBase, ProjectUtilsMixin):
  ...
  def setup_parser(cls, parent):
    ...
    parser.add_argument(
      "--rows", dest="rows", action="extend", nargs="+", type=str, required=True
    )
