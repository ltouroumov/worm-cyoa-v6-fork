...
class MediaListTool(ToolBase, ProjectUtilsMixin):
  ...
  def setup_parser(cls, parent):
    ...
    parser.add_argument("--size-gte", dest="filter_size_gte", type=float, default=None)
...
class MediaOptimizeTool(ToolBase, ProjectUtilsMixin):
  ...
  def setup_parser(cls, parent):
    ...
    parser.add_argument("--size-gte", dest="filter_size_gte", type=float, default=None)
    parser.add_argument("--max-dim", dest="max_dim", type=str, default=None)
  ...
  def run(self, args):
    ...
    images_table = Table("obj_id", "title", "Dimensions", "Size", "Optimized", "Type")
      ...
      for image_info in project_images:
        ...
        if image_info.image_is_url:
          ...
          if not str.startswith(image_info.image_data, args.export_url):
            print(
              f"Skipped {image_info.object_id}: Wrong Prefix",
              file=report_io,
            )
            continue
          ...
          if result.error:
            pg.console.log(
              f"Skipped image: {result.skip_reason or result.error}",
              style="red",
            )
            print(
              f"Skipped {image_info.object_id}: {result.error}",
              file=report_io,
            )
            continue
  ...
  def run(self, args):
    ...
    for image_info in track(project_images, total=len(project_images)):
      ...
      if image_info.image_is_url:
        ...
        result = download_external_image(self.project, image_info, dest_dir, base_url)

        if result.error:
          error_table.add_row(image_info.object_id, image_info.name, result.error)
      else:
        # Extract embedded image
        result = extract_embedded_image(self.project, image_info, dest_dir, base_url)
    ...
    for image_info in track(project_images, total=len(project_images)):
      ...
      if image_info.image_is_url:
        migrated = migrate_image_url(
          self.project, image_info, args.old_base_url, args.new_base_url
        )
        if migrated:
          migrated_count += 1
...
class MediaCleanTool(ToolBase, ProjectUtilsMixin):
  ...
  def setup_parser(cls, parent):
    parser = parent.add_parser(
      cls.name, help="Remove orphan images from export directory"
    )
    parser.add_argument("--project", dest="project_file", type=Path)
    parser.add_argument("--export-dir", type=Path, required=True)
    parser.add_argument("--export-url", type=str, required=True)
  ...
  def run(self, args):
    ...
    for image_info in project_images:
      ...
      if image_info.image_is_url and str.startswith(
        image_info.image_data, args.export_url
      ):
        ...
        if image_path not in all_image_files:
          console.log(
            f"Missing Image: {repr(image_path)} ({image_info.image_data})",
            style="red",
          )
  ...
  def run(self, args):
    ...
    with Progress() as pg:
      ...
      for image_info in project_images:
        ...
        if (
          image_info.image_data is None
          or not image_info.image_is_url
          or not str.startswith(image_info.image_data, args.export_url)
        ):
          continue
      ...
      for row_id, images in sorted(images_per_row.items()):
        ...
        for image_info in images:
          ...
          archive.write(filename=image_path, arcname=f"row-{row_id}/{image_name}")
