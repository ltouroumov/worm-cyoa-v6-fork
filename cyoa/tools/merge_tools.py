...
def diff_sequence(
  ...
  def hash_objects(seq):
    return [(obj["id"], obj_hash(obj)) for obj in seq]
  ...
  for tag, a_start, a_end, b_start, b_end in seq_match.get_opcodes():
    if tag == "equal":
      ...
      # List shrunk
      old_rows = list(map(operator.itemgetter(0), seq_a_hash[a_start:a_end]))
      new_rows = list(map(operator.itemgetter(0), seq_b_hash[b_start:b_end]))
...
SPECIAL_DISPLAY = {
    ...
    *intercalate(
      ...
      [
        Text.assemble(
          ...
          score["id"],
          ")",
          " (cond)" if len(score["requireds"]) > 0 else "",
  ...
  "requireds": lambda items: Text.assemble(
    *intercalate(
      "\n",
      [
        Text.assemble(
          ...
          item["reqId"]
          if item["type"] == "id"
          else str.join(", ", [n["req"] for n in item["orRequired"]])
          if item["type"] == "or"
          else "Other",
          " ",
          item["afterText"],
...
def update_dict(old_data: dict, new_data: dict):
  ...
  for key in chain(SPECIAL_KEYS, rest_keys):
    if key in IGNORE_KEYS:
      ...
      diff_table.add_row(
        key, show_value(old_data[key], key), show_value(new_data[key], key)
      )
      result_dict[key] = new_data[key]
      result_changed |= True
    elif key in old_data and key not in new_data:
      diff_table.add_row(
        key, show_value(old_data[key], key), Text("N/A", style="grey50")
      )
      result_changed |= True
    elif key not in old_data and key in new_data:
      diff_table.add_row(
        key, Text("N/A", style="grey50"), show_value(new_data[key], key)
      )
      result_dict[key] = new_data[key]
      result_changed |= True
    elif key in IMPORTANT_KEYS and key in old_data:
      diff_table.add_row(
        key, show_value(old_data[key], key), Text("==", style="grey50")
      )
      result_dict[key] = old_data[key]
...
class ProjectMergeTool(ToolBase, ProjectUtilsMixin):
  ...
  def setup_parser(cls, parent):
    ...
    parser.add_argument("--patch", dest="patch", type=Path, required=True)
    parser.add_argument("--write", dest="write", action="store_true")
    parser.add_argument("--quiet", dest="verbose", action="store_false")
    parser.add_argument("--equal", dest="equal", action="store_true")

    parser.add_argument(
      "--skip-rows", dest="skip_rows", nargs="+", action="extend", default=[]
    )
    parser.add_argument(
      "--skip-rows-data",
      dest="skip_rows_data",
    ...
    parser.add_argument(
      "--skip-objs", dest="skip_objs", nargs="+", action="extend", default=[]
    )

    parser.add_argument(
      "--only-rows", dest="only_rows", nargs="+", action="extend", default=[]
    )
    parser.add_argument(
      "--only-objs", dest="only_objs", nargs="+", action="extend", default=[]
    )
  ...
  def run(self, args):
    ...
    def update_object(old_obj, new_obj):
      ...
      if should_skip:
        if args.verbose:
          console.print(
            f"  Skipped Updated Item ({old_obj['id']}): {old_obj['title']}",
    ...
    def delete_object(items):
      ...
      for item in items:
        ...
        console.print(f"  Deleted Item ({item['id']}): {item['title']}", style="red")
        if args.verbose:
          console.print(diff_table)
    ...
    def insert_object(new_items):
      ...
      for item in new_items:
        ...
        if should_skip:
          if args.verbose:
            console.print(
              f"  Skipped Inserted Item ({item['id']}): {item['title']}",
              style="dark_slate_gray1 italic",
            )
          continue

        _, _, diff_table = update_dict(item, item)
        console.print(f"  Inserted Item ({item['id']}): {item['title']}", style="green")
        if args.verbose:
          console.print(diff_table)
    ...
    def update_row(old_row, new_row):
      ...
      if obj_hash(old_row) != obj_hash(new_row):
        should_skip_data = (
          old_row["id"] in args.skip_rows_data or len(args.skip_rows_data) > 0
        )
    ...
    def delete_row(rows):
      ...
      for row in rows:
        ...
        console.print(f"Deleted Row ({row['id']}): {row['title']}", style="red")
        if args.verbose:
          console.print(diff_table)
    ...
    def insert_row(new_rows):
      ...
      for row in new_rows:
        ...
        if should_skip:
          if args.verbose:
            console.print(
              f"Skipped Inserted Row ({row['id']}): {row['title']}",
              style="dark_slate_gray1 italic",
            )
          continue
        console.print(f"Inserted Row ({row['id']}): {row['title']}", style="green")
