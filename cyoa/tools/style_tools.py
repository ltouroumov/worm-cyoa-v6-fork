...
def diff_project_styles(project):
  ...
  def diff_styles(styling_1, styling_2):
    styling_diff = [
      (k, v) for k, v in styling_1.items() if k not in styling_2 or v != styling_2[k]
    ]
...
class StyleTransformer:
  ...
  def update_styles(self):
    self.update_properties(
      self.project["styling"], self.project_style["properties"], "project"
    )
  ...
  def apply_styles(self, obj_data, context: str):
    ...
    if (row_styling_data := self.object_styles.get(obj_ref, None)) is not None:
      ...
      for class_id, class_data in class_styling_data:
        console.print(
          f"Apply class {class_id} to '{obj_data['title']}' ({obj_data['id']})"
        )
        self.apply_object_styles(obj_data, obj_ref, class_data)
    elif obj_data.get("isPrivateStyling", False):
      console.print(
        f"Update private styles of row '{obj_data['title']}' ({obj_data['id']})"
      )
      self.update_properties(
        obj_data["styling"], self.project_style["properties"], obj_ref
      )
    elif context == "row":
      ...
      match update:
        case {"op": "replace", "value": value, "old": old_value}:
          ...
          if current_value == old_value:
            ...
            console.print(
              f"[green]Updated {context}/{prop_key} from {current_value} to {update_value}"
            )
          else:
            console.print(
        ...
        case other if style_def.is_valid(other):
          ...
          console.print(
            f"[green]Updated {context}/{prop_key} from {current_value} to {other}"
          )
        case other:
...
class StylesUpdateTool(ToolBase, ProjectUtilsMixin):
  ...
  def setup_parser(cls, parent):
    ...
    parser.add_argument("--project", dest="project_file", type=Path, required=True)
    parser.add_argument("--styling", dest="styling_file", type=Path, required=True)
    parser.add_argument("--out", dest="out_file", type=Path, required=False)
